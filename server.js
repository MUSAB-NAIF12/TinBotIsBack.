...
setInterval(() => {
  http.get(`http://plume-principled-hell.glitch.me/`);
}, 280000);
...
client.on('message', msg => {  
    if (msg.content === 'مصعب') {  
      msg.reply('** لـبـيـة <a:pp556:746203584319979563> **');  
    }
  });
  ...
  client.on('message', msg => {  
    if (msg.content === '-link') { 
   msg.reply ('<a:pp916:746201301200797706> [invite Bot](<https://discord.com/api/oauth2/authorize?client_id=747949575934967808&permissions=8&scope=bot>)');  
    }
  });
...
client.on("message", message => {    
    if(message.content.startsWith(prefix + "ايموجياااات")) {   
...
const replyForMention = [   "**Hi For More Informations Type `-help`**",  ]//Professional Codes
client.on('message', message=> {//Professional Codes
    if (message.author.bot) return;//Professional Codes
    if (message.isMentioned(client.user))//Professional Codes
    {
    message.reply(replyForMention);//Professional Codes
    }
});
...
if(command === `unmute`) {
  if(!message.member.hasPermission("ADMINISTRATOR")) return message.channel.sendMessage("**ليس لديك صلاحية لفك عن الشخص ميوت**:x: ").then(m => m.delete(5000));
...
client.on('message', message => {    
  ...
  client.on("message", function(message) {
   ...
   if(message.content.startsWith(prefix + "help")) {
    ...
    .addField("Admin | ادمنيه","⚙️",true)
    .addField("Games | العاب","🎯",true)
    .addField("BotRooms | رومات البوت","🧿",true)
      .setDescription(`
[server support](https://discord.gg/fywqPK5) | [invite bot](https://discord.com/api/oauth2/authorize?client_id=747949575934967808&permissions=8&scope=bot)
`)
    message.channel.send(RpsEmbed).then(msg => {
        msg.react('💡')
        msg.react("⚙️")
...
『-perms ====> لعرض صلاحياتك
『-top-inv ====> لعرض صاحب اكثر دعوات 
『-to-owner-bot ====> للتواصل مع صاحب البوت
『-clan ====> لعبة الكلانات
『-report ====> للابلاغ عن احد
『-الدعوات ====> لمعرفة عدد دعواتك
『-color ====> لوضع لون لك
『-emojilist ====> لرؤية قائمة اموجيات السيرفر
『-history ====> يعرضلك تاريخ اليوم
『-roleinfo ====> يوريك معلومات رتبة باسمها او الايدي حقها
『-say ====> يكرر الكلام الي تكتبه
『-avatar ====> صورتك او صورة الي تمنشنه
『-avt ====> يعطيك صورة الشخص عن طريق الايدي
『-avatar-server ====> صورة السيرفر
『-user ====> معلومات عنك
『-new ====> يفتح لك تكت
『-embed ====> يكرر الي تقولو بشكل حلو
『-rules ====> يعرض لك قوانين السيرفر
『-link ====> لدعوة البوت الى سيرفرك
**
`)
   message.author.sendEmbed(embed)
      message.reply('تم ارسال الاوامر العامة بالخاص')
})
reaction2.on("collect", r => {
      const embed = new Discord.RichEmbed()
...
**
『-setbots ====> لعمل روم صوتي بعدد البوتات في السيرفر
...
『-show ====> إظهار الشات
『-hide ====> اخفاء الشات
『-talking ====> للتكلم بصفة البوت
『-count ====> عدد اعضاء السيرفر
『-setlog ====> لصنع روم اللوج
『-server ====> يعرض لك معلومات عن السيرفر
『-mvall ====> لسحب الجميع الى رومك
『-sug ====> suggestions لصنع اقتراح لازم روم 
『-servers-bot ====> لمعرفة سيرفرات البوت
『-rooms ====> لرؤية رومات السيرفر
『-all-bots ====> يوريك كل البوتات في سيرفرك
『-move ====> لسحب عضو الى روم صوتية
『-giveaway ====> لصنع جيفواي
『-role @mention  ====> لأعطاء رتبة لعضو معين
『-role all  ====> لأعطاء رتبة للجميع
『-role humans  ====> لأعطاء رتبة للاشخاص فقط
『-role bots  ====> لأعطاء رتبة لجميع البوتات
『-status ====> حالات اعضاء السيرفر
『-مسح <numbr> ====> مسح الشات بعدد
『-clear ====> مسح الشات
『-mute @user <reason> ====> اعطاء العضو ميوت
...
『-lock ====> تقفيل الشات
『-unlock ====> فتح الشات
『-addem  ====> يحط لك صورة الشخص ايموجي
『-setname  ====> يغير لك اسم السيرفر
『-ct <name> ====> انشاء شات
『-cv <name> ====> انشاء رووم فويس
**
`)
   message.author.sendEmbed(embed)
      message.reply('تم ارسال الاوامر الاداريه بالخاص')
})
reaction3.on("collect", r => {
  const embed = new Discord.RichEmbed()
...
`)
   message.author.sendEmbed(embed)
   message.reply('تم ارسال اوامر الالعاب بالخاص')
})
...
client.on("message", async message => {
            if(!message.channel.guild) return;
        if(message.content.startsWith(prefix + 'الدعوات')) {
        var nul = 0
...
client.on('message', message => {
    if (message.content.startsWith(prefix + 'sug')) {
        ...
        let Room = message.guild.channels.find(`name`, "develpors")
...
client.login("NzQ3OTQ5NTc1OTM0OTY3ODA4.X0WUUw.4jOUJPjJzbs_9koI5pdtWvniInE");